...
export default function ChatBubbleCustomizer({ open, onClose, onSave, initialSettings }) {
    ...
    export default function ChatBubbleCustomizer({ open, onClose, onSave, initialSettings }) {
        ...
        const convertOpacityToHex = (opacity) => Math.round(opacity * 255).toString(16).padStart(2, '0');
        ...
        const getPreviewStyle = () => {
            ...
            const opacityHex = convertOpacityToHex(settings.opacity);

            if (settings.type === 'solid') {
                return {
                    ...baseStyle,
                    backgroundColor: `${settings.color1}${opacityHex}`
                };
            } else {
                return {
                    ...baseStyle,
                    background: `linear-gradient(${settings.gradientDirection}, ${settings.color1}${opacityHex}, ${settings.color2}${opacityHex})`
                };
            }
        };

        const handleOpacityChange = debounce((_, value) => {
            setSettings(prev => ({ ...prev, opacity: value }));
        }, 300);

        const handleBlurChange = debounce((_, value) => {
    ...
    ChatBubbleCustomizer.defaultProps = {
        initialSettings: {
            type: 'solid',
            color1: '#1a1a40',
            color2: '#4a4a80',
