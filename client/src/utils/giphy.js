const GIPHY_API_KEY = process.env.REACT_APP_GIPHY_API_KEY || 'DO7ARGJtRRks2yxeAvolAIBFJqM74EPV';
...
export const getTrendingGifs = async (limit = 20) => {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Error fetching trending GIFs: ${response.statusText}`);
        }
        const data = await response.json();
        return data.data;
    } catch (error) {
